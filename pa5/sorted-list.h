...
/*made this so each node can have 3 parts*/
typedef struct node_{
	int count; /*refrence counter for the node*/
	void* data; /*pointer to the word the node holds*/
	void* next; /*pointer to next word node*/
}node;
...
struct SortedList {
	int totalsize; /*total number of items on the list*/
	node* start;    /*pointer to node at start of list*/
	CompareFuncT cmp; /*to store the value of the cmp function gives out*/
	DestructFuncT dst;
};
